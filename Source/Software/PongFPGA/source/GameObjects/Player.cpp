...
Player::Player( const int windowHight, const Vector2i batSize,
		...
		m_rect( leftSide ? batSize.x : WINDOW_WIDTH - 2 * batSize.x,
		        (WINDOW_HEIGHT / batSize.y) / 2, batSize.x, batSize.y ),
...
void Player::update( )
{
	...
	if( updateDelayCounter > 16 )
	{
		...
		if( m_AIAimRect )
		{
			if( m_rect.top + m_rect.height / 2
			        > m_AIAimRect->top + m_AIAimRect->height / 2 )
			{
				processInput( m_leftSide ? LBUTTON : RBUTTON );
			}
			else if( m_rect.top + m_rect.height / 2
			        < m_AIAimRect->top + m_AIAimRect->height / 2 )
			{
		...
		if( ((m_rect.top + m_rect.height < m_windowHight || m_velocity < 0)
